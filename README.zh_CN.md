...
- `RunningProgressLoop` 和 `RunningProgressNotifier`：成对使用的后台
  running 进度汇报辅助对象，适合 worker 线程更新共享业务状态、后台汇报线程读取快照并发送事件。
...
时，使用 `RunningProgressLoop`。worker 继续维护自己的业务状态；后台循环只负责等待
超时或 `RunningProgressNotifier::running_point` 信号，然后调用你提供的快照闭包，
把当前业务状态转换为新的 `ProgressCounters`。
...
如果汇报间隔是正数，worker 通常不必发送 `running_point`；后台循环会通过
`recv_timeout` 定时醒来。操作结束时仍应发送 stop 信号，并在汇报 `finished`、
`failed` 或 `canceled` 终态事件前 join 后台线程。
