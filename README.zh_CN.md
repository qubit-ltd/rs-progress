...
- `RunningProgressLoop`、`ScopedRunningProgress` 和
  `RunningProgressPoints`：后台 running 进度汇报辅助对象，适合 worker
  线程更新共享业务状态、后台汇报线程读取快照并发送事件。
...
当工作在一个或多个 worker 线程中执行，但 reporter 回调希望放到单独的后台汇报线程中
时，使用 `RunningProgressLoop::spawn_scoped`。worker 继续维护自己的业务状态；
后台循环只负责等待超时或 `RunningProgressPoints::running_point` 信号，然后调用
你提供的快照闭包，把当前业务状态转换为新的 `ProgressCounters`。

这种模式适合并行执行器：正数汇报间隔下，worker 的热路径不需要直接调用 reporter；
当间隔是 `Duration::ZERO` 时，又可以在每个 worker 进度点之后唤醒后台循环，而且不会
忙等。协调线程持有 `ScopedRunningProgress` guard，worker 只拿
`RunningProgressPoints`，结束时由协调线程调用 `stop_and_join`。
...
如果汇报间隔是正数，`RunningProgressPoints::running_point` 是 no-op；后台循环会
通过 `recv_timeout` 定时醒来。这样 worker 可以无条件调用它，同时 stop/join 仍由
协调线程持有的 guard 负责。
...
- `RunningProgressLoop`：由超时或 worker 信号驱动的后台 running 事件循环。
- `ScopedRunningProgress`：持有 scoped 后台汇报线程的生命周期 guard。
- `RunningProgressPoints`：可克隆的 worker 侧 running point 句柄。
- `RunningProgressNotifier`：可克隆的唤醒/停止 `RunningProgressLoop` 句柄。
