...
本库把这两类状态分开：启动时仅配置一次 `Metric` 的稳定 ID、显示名称和可选总量；随后由 `Progress` 持有动态计数，可克隆的指标句柄以经过校验的生命周期转换更新它们，每个事件都读取一个内部一致的快照。包含多个指标时，只保证每个指标快照各自一致；运行中的多个指标之间不是全局原子视图。终态方法会消费 `Progress`，因此在安全 Rust 中至多发送一个终态事件，且不能在终态后继续上报；但若在调用终态方法前丢弃对象或发生 unwind，操作仍可能没有终态事件。
