...
- `RunningProgressLoop`、`ScopedRunningProgress` 和
  `RunningProgressPointHandle`：后台 running 进度汇报辅助对象，适合
  worker 线程更新共享业务状态、后台汇报线程读取快照并发送事件。
...
后台循环只负责等待超时或 `RunningProgressPointHandle::report` 信号，然后调用
...
如果汇报间隔是正数，`RunningProgressPointHandle::report` 是 no-op；后台循环会
